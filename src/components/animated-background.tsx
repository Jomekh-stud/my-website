...
export function AnimatedBackground() {
  ...
  const entranceOffsetRef = useRef<number>(0);
  const entranceStartTimeRef = useRef<number>(0);
  const shapeInitialYRef = useRef<Map<HTMLElement, { top: number; height: number }>>(new Map());
  ...
  const updateShapes = useCallback(() => {
    ...
    const scrollY = prefersReducedMotion ? 0 : shouldUseFakeScroll ? fakeScrollRef.current : window.scrollY;
    ...
    shapes.forEach((shape) => {
      ...
      const desiredRotation = prefersReducedMotion ? 0 : (scrollY * rotationFactor * 0.05) % 360;
      ...
      // Entrance animation offset with vertical wrapping
      let entranceY = 0;
      if (entrancePhaseRef.current !== "done") {
        const rawOffset = entranceOffsetRef.current;
        ...
        if (initialData && rawOffset !== 0) {
          ...
          // Modular wrap: keeps shape cycling through the viewport
      ...
      element.style.transform = `translate3d(${repelX}px, ${yMovement + repelY + entranceY}px, 0) rotate(${rotation}deg)`;
    });
  ...
  useEffect(() => {
    ...
    // Initialize entrance animation if first visit this session
    const alreadyPlayed =
      typeof sessionStorage !== "undefined" && sessionStorage.getItem("bg-entrance-played");

    if (!prefersReducedMotion && !alreadyPlayed) {
      // Capture initial positions before any transforms
      const initialPositions = new Map<HTMLElement, { top: number; height: number }>();
      shapes.forEach((shape) => {
        const rect = shape.getBoundingClientRect();
        initialPositions.set(shape, { top: rect.top, height: rect.height });
      });
      shapeInitialYRef.current = initialPositions;

      // Disable CSS transitions during entrance (they fight with RAF updates)
      shapes.forEach((shape) => {
  ...
  }, [isLargeScreen, prefersReducedMotion, scheduleUpdate]);

  // Entrance animation loop
  useEffect(() => {
    if (entrancePhaseRef.current === "done") return;

    const SCROLL_SPEED = 0.8; // px per ms — how fast blobs scroll
    const SCROLL_DURATION = 2200; // ms — how long the animation lasts
    const SCROLL_DISTANCE = SCROLL_SPEED * SCROLL_DURATION;

    let rafId: number;

    const tick = (now: number) => {
      const phase = entrancePhaseRef.current;
  ...
  }, [updateShapes, entranceTrigger]);

  const retriggerEntrance = useCallback(() => {
    const shapes = shapeElementsRef.current;
    if (!shapes.length) return;

    // Reset offset to 0 first so getBoundingClientRect captures true positions
    entrancePhaseRef.current = "done";
    entranceOffsetRef.current = 0;
    updateShapes();
    ...
    // Bump trigger to restart the RAF effect
    setEntranceTrigger((n) => n + 1);
  }, [updateShapes]);
  ...
  return (
    <>
      {process.env.NODE_ENV === "development" && (
        <button
          onClick={retriggerEntrance}
          className="fixed top-2 right-2 rounded bg-black/60 px-2 py-1 text-xs text-white opacity-40 hover:opacity-100"
          style={{ zIndex: 9999 }}
          type="button"
        >
          replay bg
